...
class Runtime {
    ...
    mkAssetCreateTx(name, flags, asaDef) {
        ...
        if (this.ctx.tx === undefined || this.ctx.tx.type !== constants_1.TransactionTypeEnum.ASSET_CONFIG) {
    ...
    addCtxAppCreateTxn(creator, appDef, payFlags) {
        ...
        appDef.localInts, appDef.localBytes, appDef.globalInts, appDef.globalBytes, web_1.parsing.parseAppArgs(appDef.appArgs), appDef.accounts, appDef.foreignApps, appDef.foreignAssets, appDef.note, appDef.lease, payFlags.rekeyTo);
        const encTx = { ...txn.get_obj_for_encoding(), txID: txn.txID() };
        this.ctx.tx = encTx;
        this.ctx.gtxs = [encTx];
    }
    ...
    deployApp(sender, appDefinition, payFlags, scTmplParams, debugStack) {
        ...
        this.ctx.budget = constants_1.MAX_APP_PROGRAM_COST;
    ...
    executeTx(txnParams, debugStack) {
        ...
        let tx, gtxs;
        if (web_1.types.isSDKTransactionAndSign(txnParams[0])) {
            const sdkTxns = txnParams.map((txnParamerter) => {
                const txn = txnParamerter;
                return txn.transaction.get_obj_for_encoding();
            });
            tx = sdkTxns[0];
            gtxs = sdkTxns;
        }
            ...
            [tx, gtxs] = this.createTxnContext(txns);
        }
        ...
        const runtimeTxnParams = txnParams.map((txn) => web_1.types.isSDKTransactionAndSign(txn) ? (0, txn_1.transactionAndSignToExecParams)(txn, this.ctx) : txn);
        // calculate budget for single/group tx
        const applCallTxNumber = gtxs.filter((txn) => txn.type === constants_1.TransactionTypeEnum.APPLICATION_CALL).length;
        this.ctx.budget = constants_1.MAX_APP_PROGRAM_COST * applCallTxNumber;
