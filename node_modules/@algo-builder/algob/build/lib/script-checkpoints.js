...
function toScriptFileName(filename) {
    filename = filename.replace(artifactsPath + String(path_1.default.sep), "");
...
function appendToCheckpoint(checkpoints, networkName, append) {
    ...
    const allAssetNames = [...append.asa.keys(), ...append.app.keys()];
    for (const assetName of allAssetNames) {
        if ((orig.asa.get(assetName) &&
            !(0, deep_equal_1.default)(orig.asa.get(assetName), append.asa.get(assetName))) ??
            (orig.app.get(assetName) &&
                !(0, deep_equal_1.default)(orig.app.get(assetName), append.app.get(assetName)))) {
            throw new web_1.BuilderError(web_1.ERRORS.BUILTIN_TASKS.CHECKPOINT_ERROR_DUPLICATE_ASSET_DEFINITION, {
                assetName: assetName,
            });
        }
    }
    orig.asa = new Map([...orig.asa, ...append.asa]);
    orig.app = new Map([...orig.app, ...append.app]);
...
class CheckpointRepoImpl {
    ...
    isDefined(networkName, name) {
        ...
        return (netCP !== undefined &&
            (netCP.asa.get(name) !== undefined ||
                netCP.app.get(name) !== undefined ||
...
async function registerCheckpoints(deployer, execParams, txns, txIdxMap) {
            ...
            case "appl": {
                ...
                if (res) {
                    const sscInfo = {
                        ...
                        deleted: false,
                        approvalFile: execParams[idx].approvalProgram,
                        clearFile: execParams[idx].clearProgram,
                    };
...
function convertCPValsToMaps(cpWithObjects) {
    ...
    cpWithObjects.app = toSSCMap(cpWithObjects.app);
...
function findCheckpointsRecursive() {
    return (0, runtime_1.lsTreeWalk)(ensureCheckpointsPath()).filter((filename) => filename.endsWith(checkpointFileSuffix));
}
...
class CheckpointFunctionsImpl {
     ...
     */
    getAppfromCPKey(key) {
        const resultMap = this.cpData.precedingCP[this.networkName]?.app ?? new Map();
